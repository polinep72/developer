...
# Добавляем корень проекта в PYTHONPATH для импорта wsb_core
PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

# Импорты из wsb_core (общее ядро)
try:
    ...
    from wsb_core.bookings_core import extend_booking_core
except ImportError:
    # Fallback на локальные константы, если wsb_core недоступен
...
# Используем константы из wsb_core, если доступны
if 'WORKING_HOURS_START' in globals():
    const.WORKING_HOURS_START = WORKING_HOURS_START
    const.WORKING_HOURS_END = WORKING_HOURS_END
    const.BOOKING_TIME_STEP_MINUTES = BOOKING_TIME_STEP_MINUTES
    if hasattr(const, 'MAX_BOOKING_DURATION_HOURS'):
        const.MAX_BOOKING_DURATION_HOURS = MAX_BOOKING_DURATION_HOURS
...
# --- Новая или модифицированная функция для HTML-форматирования одной брони ---
def format_booking_info_html(
    ...
    """Форматирует информацию о бронировании в HTML-строку."""

    s_equip_name = equip_name if equip_name else "Неизвестное оборудование"
    s_user_fi = user_fi if user_fi else "Неизвестный пользователь"
    s_date = _format_date(booking_date)  # Используем вашу функцию _format_date
    ...
    # 💻 Оборудование, 👤 Пользователь, 🗓️ Дата, ⏰ Время
    return (
        f"💻 <b>{s_equip_name}</b>\n"
        f"👤 Пользователь: {s_user_fi}\n"
        f"🗓️ Дата: {s_date}\n"
        f"⏰ Время: {s_time_start} - {s_time_end}"
    )


# --- Модифицированная функция get_bookings_by_date_text ---
def get_bookings_by_date_text_html(db: Database, selected_date: date) -> str:  # Переименовал для ясности
    """Форматирует бронирования на дату в HTML-текст, включая ссылку на тепловую карту."""

    bookings: List[BookingRow] = get_bookings_by_date(db, selected_date)
    date_str_formatted = _format_date(selected_date)  # Используем вашу _format_date

    # Формируем ссылку на тепловую карту
    heatmap_url = config.HEATMAP_BASE_URL
    if not heatmap_url:
        logger.warning("HEATMAP_BASE_URL не настроен в конфигурации.")
        heatmap_link_html = ""
    else:
        # Вариант 1: Ссылка на главную страницу карты, пользователь выбирает дату там
        #heatmap_link_html = (
        #    f"📊 <a href='{heatmap_url}'>Тепловая карта занятости</a>\n"
        #    f"(на карте при необходимости выберите дату {date_str_formatted})\n"
        #)
        # Вариант 2: Если ваше Dash-приложение может принимать дату как параметр
        date_for_link = selected_date.strftime('%Y-%m-%d')  # <--- КЛЮЧЕВАЯ СТРОКА
        heatmap_url_with_date = f"{heatmap_url}?date={date_for_link}"  # <--- ФОРМИРОВАНИЕ URL С ПАРАМЕТРОМ
        heatmap_link_html = f"📊 <a href='{heatmap_url_with_date}'>График занятости на {date_str_formatted}</a>\n"

    # Собираем сообщение
    if not bookings:
        message_text = f"На {date_str_formatted} нет активных бронирований.\n"
        if heatmap_link_html:
            message_text += f"\n{heatmap_link_html}"
        return message_text

    # Используем HTML для всего сообщения
    response_lines = [f"<b>Бронирования на {date_str_formatted}:</b>"]
    if heatmap_link_html:
        response_lines.append(heatmap_link_html)

    for booking in bookings:
        # Передаем данные в HTML-форматтер
        response_lines.append(format_booking_info_html(
            equip_name=booking.get('name_equip'),
            booking_date=booking.get('date'),  # Это поле date из таблицы bookings
            time_start=booking.get('time_start'),
            time_end=booking.get('time_end'),
            user_fi=booking.get('user_fi')
        ))
        response_lines.append("<pre>--------------------</pre>")  # HTML-совместимый разделитель

    # Удаляем последний разделитель, если он есть и если были брони
    ...
    if len(response_html) > const.MAX_MESSAGE_LENGTH:
        # Учитываем, что HTML-теги тоже занимают место, но для простоты обрезаем как есть
        response_html = response_html[:const.MAX_MESSAGE_LENGTH - 40] + "\n... (список слишком длинный)"
        logger.warning(f"HTML список броней на {date_str_formatted} обрезан.")
...
def get_bookings_by_workspace_text(db: Database, equipment_id: int, equipment_name: str) -> str:
    ...
    if not bookings: return f"Для '{equipment_name}' нет броней."
...
def calculate_available_slots(
    ...
    Использует унифицированную логику из wsb_core.slots.
    """
    logger.debug(f"Расчет слотов для equip={equipment_id} на {selected_date}")
    
    # Пытаемся использовать унифицированную функцию из wsb_core
    try:
        from wsb_core.slots import calculate_available_slots_from_bookings
        
        all_bookings_on_date = get_bookings_by_date(db, selected_date)
        slots = calculate_available_slots_from_bookings(
            bookings=all_bookings_on_date,
            equipment_id=equipment_id,
            selected_date=selected_date,
            now_dt=datetime.now()
        )
        logger.info(f"Итоговые слоты для {equipment_id} на {selected_date}: {slots}")
        return slots
    except ImportError:
        # Fallback на локальную реализацию, если wsb_core недоступен
        logger.warning("wsb_core.slots недоступен, используется локальная реализация")
        return _calculate_available_slots_local(db, equipment_id, selected_date)


def _calculate_available_slots_local(
    db: Database,
    equipment_id: int,
    selected_date: date
) -> List[Tuple[time, time]]:
    """
    Локальная реализация расчета слотов (fallback, если wsb_core недоступен).
    """
    all_bookings_on_date = get_bookings_by_date(db, selected_date)
    equipment_bookings = [
...
def create_booking(db: Database, user_id: int, equipment_id: int, selected_date_str: str, start_time_str: str, duration_str: str) -> Tuple[bool, str, Optional[int]]:
    ...
    except Exception as e_parse: # <<< ИСПРАВЛЕНО >>>
...
def extend_booking(db: Database, booking_id: int, user_id: int, extension_str: str) -> Tuple[bool, str]:
    ...
    При наличии wsb_core использует extend_booking_core, иначе — локальную реализацию.
    """
    # 1. Парсинг строки длительности (как и раньше)
    try:
        if not extension_str or ":" not in extension_str:
            raise ValueError("Некорр. формат HH:MM")
        h, m = map(int, extension_str.split(":"))
        total_minutes = h * 60 + m
        if h < 0 or m < 0 or total_minutes == 0 or total_minutes % const.BOOKING_TIME_STEP_MINUTES != 0:
            raise ValueError(f"Некорр. длит./не кратна {const.BOOKING_TIME_STEP_MINUTES} мин.")
        ...
        if conn is not None:
            try:
                with conn.cursor() as cur:
                    ...
                    core_result: BookingCoreResult = extend_booking_core(
                        cur,
                        booking_id=booking_id,
                        actor_user_id=user_id,
                        extension_minutes=total_minutes,
                        is_admin=False,
                        sync_slots=False,
                    )
    ...
    b_finish_time = booking_info.get("finish")
    equip_name = booking_info.get("equipment_name", "???")

    if b_user_id != user_id:
        logger.warning(f"{user_id} продляет чужую {booking_id}.")
        return False, "Это не ваше бронирование."
    if b_cancel:
        return False, "Бронь отменена."
    if b_finish_time is not None:
        return False, "Бронь завершена."
    if not isinstance(cur_end, datetime) or not isinstance(time_start, datetime):
        logger.error(f"{booking_id}: некорр. время.")
        return False, const.MSG_ERROR_GENERAL
    cur_end_naive = cur_end.replace(tzinfo=None)
    time_start_naive = time_start.replace(tzinfo=None)
    now_naive = datetime.now().replace(tzinfo=None)
    if cur_end_naive <= now_naive:
        logger.warning(f"{user_id} продляет закончившуюся {booking_id}")
        return False, "Бронирование уже завершилось."

    new_end_dt = cur_end + extend_delta
    new_end_naive = new_end_dt.replace(tzinfo=None)
    current_date = cur_end.date()
    work_end_dt = datetime.combine(current_date, const.WORKING_HOURS_END).replace(tzinfo=None)
    if new_end_naive > work_end_dt:
        logger.warning(f"Продление {booking_id} {user_id} > раб. дня ({new_end_naive} > {work_end_dt}).")
    ...
    try:
        ...
        if conflicts:
            c = conflicts[0]
            c_s = _format_time(c.get("time_start"))
            c_e = _format_time(c.get("time_end"))
            c_u = c.get("user_fi", "???")
            msg = const.MSG_BOOKING_FAIL_OVERLAP + f"\n(Занято: {c_u}, {c_s} - {c_e})"
...
def get_all_active_bookings_for_admin_display(db: Database) -> Optional[List[Dict[str, Any]]]:
    """
    Получает все активные бронирования для отображения администратору.
    ...
    try:
        ...
        # execute_query возвращает List[Dict[str, Any]] или None
        return results if results else [] # Возвращаем пустой список, если нет данных
    except Exception as e:
        logger.error(f"Ошибка при получении активных броней для админа (display): {e}", exc_info=True)
        return None # Или [] в зависимости от того, как хотите обрабатывать ошибку выше
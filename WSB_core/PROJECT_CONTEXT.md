...
- `WSB/` — боевой Telegram‑бот (версия 6.0.2).
...
**Текущая версия ядра:** 1.0.4 (исправления UI и совместимости библиотек)
...
- **Отправка email (`wsb_core.email_service`):**
  - Модуль `email_service.py` предоставляет базовую функциональность отправки email-уведомлений через SMTP.
  - Конфигурация SMTP читается из `.env` через `app_config.py` (WSB_SMTP_HOST, WSB_SMTP_PORT, WSB_SMTP_USER, WSB_SMTP_PASSWORD, WSB_SMTP_FROM_EMAIL, WSB_SMTP_FROM_NAME, WSB_ENABLE_EMAIL).
  - Функция `send_booking_created_notification()` отправляет уведомление о создании бронирования с HTML и текстовым форматом.

- **Отслеживание новых бронирований (`wsb_core.booking_watcher`):**
  - Модуль `booking_watcher.py` отслеживает новые бронирования в БД (созданные через бот) и автоматически отправляет email-уведомления.
  - Использует механизм отслеживания последнего обработанного ID для избежания дублей.
  - Интегрирован в бот через планировщик (проверка каждые 30 секунд).
...
9. 🔄 Постепенно использовать `wsb_core.notifications_logic` в e‑mail‑уведомлениях портала (единые интервалы и проверки).
   - ✅ Константы используются в `booking_reminders.py`
   - ❌ Функции валидации (`is_booking_valid_for_start_notification`, `is_booking_valid_for_end_notification`) не используются в `notification_worker.py`
10. ⏳ Протестировать работу бота и портала из `WSB_core` на одном и том же бронировании (создание → продление в портале → продление в боте → отмена).
    - ✅ Есть тестовый скрипт для расписания уведомлений
    - ❌ Нет комплексного тестирования полного цикла бронирования
11. 🔄 После стабилизации — подготовить Docker‑окружение и сценарий миграции на ядро без остановки боевых систем.
    - ✅ Есть базовый `docker-compose.yml`
    - ❌ Ссылается на боевые версии (`../WSB/`, `../WSB_portal/`) вместо `WSB_core/`
    - ❌ Нет Dockerfile для `wsb_bot` в `WSB_core`
    - ❌ Нет сценария миграции

**Легенда:** ✅ Выполнено | 🔄 Частично выполнено | ⏳ Не выполнено

**Подробный отчет:** см. `STATUS_REPORT.md`

...
# Если finish отсутствует, заполняем его значением time_end
...
# Приводим дату бронирования к типу date
df["Date"] = pd.to_datetime(df["Date"]).dt.date

# Список уникальных приборов
...
app.layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Input(id="target-load", type="number", value=8)
    ], className="filters"),
...
def update_dashboard(selected_equipments, start_date, end_date, target_load):
    ...
    # Добавляем сетку и вертикальные линии (разделитель дней в 00:00)
    ...
    # Добавляем вертикальные линии для каждой даты (00:00)
    shapes = []

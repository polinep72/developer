...
# Заполняем finish значением time_end, если finish отсутствует
...
def update_dashboard(selected_equipments, start_date, end_date, target_load):
    ...
    # Фильтрация исходного DataFrame по выбранным приборам и датам
    filtered_df = df[
    ...
    # Группировка по дате и прибору (Hours_Used уже посчитан)
    daily_usage = (
        filtered_df
    ...
    # Создаём полный диапазон дат в выбранном периоде
    all_dates = pd.date_range(start_date_obj, end_date_obj, freq='D').date
    # Создаём таблицу с полным каркасом: все комбинации выбранных приборов и всех дней
    all_combinations = pd.DataFrame(list(itertools.product(all_dates, selected_equipments)),
                                    columns=["Date", "Equipment"])
    # Объединяем полученную группировку с полным набором комбинаций
    daily_usage = pd.merge(all_combinations, daily_usage, on=["Date", "Equipment"], how="left")
    # Если для каких-то комбинаций отсутствуют данные, заполняем нулями
    daily_usage["Hours_Used"] = daily_usage["Hours_Used"].fillna(0)
    daily_usage["Relative_Load"] = (daily_usage["Hours_Used"] / target_load) * 100

    # График относительной загрузки с сеткой и легендой "Оборудование"
    ...
    # Добавляем сетку на оси X и Y, dtick=86400000 мс (1 день)
    ...
    # График абсолютной загрузки (без дополнительной обработки)
    fig_absolute = px.bar(
        daily_usage,
        x="Date",
        y="Hours_Used",

...
def update_dashboard(selected_equipments, start_date, end_date, target_load):
    ...
    # Таблица пользователей с округлением значений до двух знаков и выравниванием по центру
    user_summary = (
    ...
    user_table = html.Table([
        ...
        html.Tbody([
            html.Tr([
                html.Td(row["User"]),
                html.Td(f"{row['Hours_Used']:.2f}", style={"textAlign": "center"})
            ]) for _, row in user_summary.iterrows()
        ])
    ], style={"marginLeft": "15px"})

    # Таблица суммарной наработки с округлением значений до двух знаков и выравниванием по центру
    equipment_summary = (

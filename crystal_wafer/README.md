...
- Подтверждение при удалении из корзины, очистке корзины, удалении пользователя
- Замена стандартных confirm() на стильные модальные окна

### Версия 1.4.11 (2025-12-16)
- Добавлены клавиатурные сокращения для всех основных страниц
- Enter для выполнения поиска и отправки форм
- Ctrl+S / Cmd+S для сохранения изменений
- Esc для закрытия сообщений и форм

### Версия 1.4.10 (2025-12-16)
- Исправлена критическая ошибка в функции поиска: фильтры теперь применяются корректно
- Исправлен поиск по шифру кристалла - теперь ищется последовательность символов, а не отдельные символы
...
- Добавлена кнопка закрытия для flash-сообщений

### Версия 1.4.8 (2025-12-16)
- Добавлено автодополнение шифров кристаллов через HTML5 datalist
- Добавлены подсказки (title) для всех полей ввода
- API endpoint для получения списка шифров кристаллов

### Версия 1.4.7 (2025-12-16)
- Добавлена система toast-уведомлений для всех шаблонов (замена alert())
- Улучшена обработка AJAX ошибок с понятными сообщениями

### Версия 1.4.6 (2025-12-16)
- Добавлена клиентская валидация форм с HTML5 и JavaScript
- Система toast-уведомлений для замены alert()
- Валидация в реальном времени для полей паролей

### Версия 1.4.5 (2025-12-16)
- Добавлена защита от перечисления пользователей
- Задержка для замедления брутфорса паролей

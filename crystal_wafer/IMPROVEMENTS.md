...
**Проблема:** Пароли хранятся и сравниваются в открытом виде (строки 1746, 1686 в `app.py`).  
**Риск:** При компрометации БД все пароли становятся доступны злоумышленнику.
...
# При логине
if check_password_hash(db_password, u_password):
    # успешная аутентификация
```

**Миграция существующих паролей:**
- Создать скрипт миграции, который хеширует все существующие пароли
- При первом входе пользователя после миграции требовать смену пароля

---

#### 2. ⚠️ Слабый SECRET_KEY по умолчанию
**Проблема:** Используется дефолтный ключ `'a_default_secret_key_change_it'` (строка 146).  
**Риск:** Позволяет злоумышленнику подделать сессионные cookie.
...
if not SECRET_KEY:
    ...
    _flask_app.logger.warning("⚠️ SECRET_KEY не установлен, сгенерирован временный. Установите SECRET_KEY в .env!")
    
_flask_app.secret_key = SECRET_KEY

# В .env файле:
# SECRET_KEY=ваш_случайный_ключ_длиной_минимум_32_символа
```

---

#### 3. ⚠️ Отсутствие защиты от CSRF
...
#### 4. ⚠️ Отсутствие rate limiting для логина
...
#### 6. Логирование действий пользователей (Audit Log)
...
#### 8. Валидация имен таблиц и столбцов
...
#### 9. Ограничение размера загружаемых файлов
...
#### 10. Защита от перечисления пользователей
...
### 2. Улучшенная обработка ошибок на клиенте
...
1. **Хеширование паролей** - КРИТИЧНО для безопасности
2. **Генерация SECRET_KEY** - КРИТИЧНО для безопасности сессий
3. **CSRF защита** - КРИТИЧНО для защиты от атак
4. **Rate limiting для логина** - КРИТИЧНО для защиты от брутфорса

### 🟠 Важно (в ближайшее время)
5. **Валидация входных данных** - Защита от XSS и некорректных данных
6. **Audit logging** - Отслеживание действий пользователей
7. **Клиентская валидация форм** - Улучшение UX
8. **Подтверждения критичных действий** - Предотвращение случайных действий
9. **Ограничение размера файлов** - Защита от DoS

### 🟡 Желательно (по возможности)
10. **Пул подключений к БД** - Улучшение производительности

...
shutdown_flag = False
MAX_RETRIES = 5 # Максимальное количество попыток перезапуска при ошибке 409
INITIAL_RETRY_DELAY = 60  # Начальная задержка перед повторной попыткой (секунды)
MAX_RETRY_DELAY = 300 # Максимальная задержка перед повторной попыткой (секунды)
    ...
    try:
        ...
        while not shutdown_flag:
            ...
            try:
                ...
                try:
                    bot.stop_polling()
                    logger.info("Polling штатно остановлен (ReadTimeout).")
                ...
                try:
                    bot.stop_polling()
                    logger.info("Polling штатно остановлен (ConnectionError).")
                except Exception as e_stop:

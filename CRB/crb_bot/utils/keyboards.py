...
# Обновлено в соответствии с constants.py
USER_BOT_COMMANDS = [
    BotCommand("start", "🚀 Старт/Перезапуск"),
    BotCommand("help", "❓ Помощь"),
    BotCommand("booking", "📅 Бронировать переговорную"), # Текст обновлен
    BotCommand("mybookings", "📄 Мои бронирования"),
    BotCommand("cancel", "❌ Отменить бронь"),
    BotCommand("finish", "🏁 Завершить бронь"), # Текст обновлен
    BotCommand("extend", "⏳ Продлить бронь"),
    BotCommand("roombookings", "🚪 Брони по переговорным"), # Команда изменена
...
def create_user_reply_keyboard() -> ReplyKeyboardMarkup:
    ...
    btn_roombookings = KeyboardButton(const.BTN_TEXT_ROOMBOOKINGS)
    ...
    markup.add(btn_mybookings, btn_roombookings, btn_datebookings) # Кнопка изменена
    return markup
...
def create_admin_reply_keyboard() -> ReplyKeyboardMarkup:
    ...
    btn_add_room = KeyboardButton(const.BTN_TEXT_ADD_ROOM)
    btn_view_rooms = KeyboardButton(const.BTN_TEXT_VIEW_ROOMS)
    btn_admin_cancel = KeyboardButton(const.BTN_TEXT_ADMIN_CANCEL)
    btn_all = KeyboardButton(const.BTN_TEXT_ALL)
    btn_broadcast = KeyboardButton(const.BTN_TEXT_BROADCAST)
    btn_manage_user = KeyboardButton(const.BTN_TEXT_MANAGE_USER)
    btn_users = KeyboardButton(const.BTN_TEXT_USERS)
    btn_schedule = KeyboardButton(const.BTN_TEXT_SCHEDULE)
    # Добавляем админские кнопки
    markup.add(btn_admin_help)
    markup.add(btn_add_room, btn_view_rooms) # Кнопки изменены
...
# Функция generate_equipment_category_keyboard УДАЛЕНА
# Функция generate_add_equipment_category_keyboard УДАЛЕНА

def generate_conference_room_keyboard(conference_rooms: List[Dict[str, Any]], callback_prefix: str) -> InlineKeyboardMarkup:
    """Генерирует клавиатуру со списком переговорных комнат."""
    markup = InlineKeyboardMarkup(row_width=1) # Комнаты по одной в ряду
    if not conference_rooms:
        no_cr_button = InlineKeyboardButton("Нет доступных переговорных комнат", callback_data=const.CB_IGNORE)
        markup.add(no_cr_button)
        # Убедимся, что кнопка отмены есть, даже если комнат нет
        _add_cancel_booking_button(markup)
        return markup

    for room in conference_rooms:
        # Используем cr_id и cr_name или другие согласованные имена полей из БД/запроса
        cr_id = room.get('id') # Предполагаем, что ID комнаты называется 'id'
        name = room.get('cr_name', 'Без имени') # Предполагаем, что имя комнаты 'cr_name'
        if cr_id is not None:
            # callback_prefix будет const.CB_BOOK_SELECT_CR при бронировании
            button = InlineKeyboardButton(text=name, callback_data=f"{callback_prefix}{cr_id}")
            markup.add(button)
        else:
            logger.warning(f"Обнаружена переговорная комната без ID при генерации клавиатуры: {room}")
...
def generate_time_keyboard_in_slot(selected_slot: Tuple[time, time], selected_date: date, callback_prefix: str) -> InlineKeyboardMarkup:
    ...
    while current_time_dt < slot_end_dt:
        ...
        is_before_slot_end = (potential_end_dt <= slot_end_dt) # Убедимся что конец в слоте
...
def generate_duration_keyboard_in_slot(start_time: time, selected_date: date, slot_end_time: time, callback_prefix: str) -> InlineKeyboardMarkup:
    ...
    while True:
        ...
        # Убедимся, что потенциальное время конца не выходит за рамки рабочего дня
        if potential_end_dt.time() > const.WORKING_HOURS_END and potential_end_dt.date() == selected_date: break
        # И за рамки слота
        if potential_end_dt > slot_end_dt: break
        # И за максимальную длительность
...
def generate_user_bookings_keyboard(bookings: List[Dict[str, Any]], callback_prefix: str) -> InlineKeyboardMarkup:
    ...
    if not bookings:
        no_bookings_button = InlineKeyboardButton("У вас нет активных или будущих бронирований", callback_data=const.CB_IGNORE)
        markup.add(no_bookings_button)
        # Добавим кнопку отмены/назад если передан префикс для контекста
        context_parts = callback_prefix.split('_')
        if len(context_parts) > 1:
            cancel_context = "_".join(context_parts[1:-1]) # Берем все кроме cb и последнего s/c/a..
            if cancel_context:
    ...
    for booking in bookings:
        b_id = booking.get('id')
        # Используем имя комнаты 'cr_name' или 'conference_room_name'
        cr_name = booking.get('cr_name', booking.get('conference_room_name', '???'))
        b_date = booking.get('date')
        b_start = booking.get('time_start')
        b_end = booking.get('time_end')

        if not all([b_id, b_date, b_start, b_end]):
            logger.warning(f"Неполные данные для бронирования {b_id} при генерации клавиатуры: {booking}")
            continue
        ...
        if len(cr_name) > max_len:
            display_name = cr_name[:max_len] + '..'

        btn_text = f"{display_name} | {date_str} | {start_str}-{end_str}"
        # Префикс будет, например, const.CB_CANCEL_SELECT_BOOKING или const.CB_FINISH_SELECT_BOOKING
        button = InlineKeyboardButton(text=btn_text, callback_data=f"{callback_prefix}{b_id}")
        markup.add(button)

    # Добавляем кнопку Назад/Отмена после списка бронирований
    if bookings:
        context_parts = callback_prefix.split('_')
        if len(context_parts) > 1:
            # Берем контекст из префикса (например, 'cancel_s_book' -> 'cancel')
            cancel_context = context_parts[1] # Второй элемент после 'cb'
            cancel_button = InlineKeyboardButton("🔙 Назад / Отмена", callback_data=f"{const.CB_ACTION_CANCEL}{cancel_context}")
            markup.add(cancel_button)

    return markup

def generate_conference_room_list_with_delete_keyboard(conference_room_list: List[Dict[str, Any]]) -> InlineKeyboardMarkup:
    """Клавиатура для просмотра и удаления переговорных комнат админом."""
    markup = InlineKeyboardMarkup(row_width=2) # Имя и кнопка удаления в одном ряду
    if not conference_room_list:
        no_cr_button = InlineKeyboardButton("Нет переговорных комнат в базе", callback_data=const.CB_IGNORE)
        markup.add(no_cr_button)
        return markup

    for room in conference_room_list:
        cr_id = room.get('id') # Предполагаем ID комнаты 'id'
        cr_name = room.get('cr_name', '???') # Предполагаем имя комнаты 'cr_name'
        if cr_id is not None:
            name_button = InlineKeyboardButton(text=f"{cr_name} (ID:{cr_id})", callback_data=const.CB_IGNORE)
            # Используем обновленный префикс для удаления комнат
            delete_button = InlineKeyboardButton(text="🗑️ Удалить", callback_data=f"{const.CB_CR_DELETE_SELECT}{cr_id}")
            markup.add(name_button, delete_button)
        else:
            logger.warning(f"Обнаружена комната без ID в списке для удаления: {room}")
...
def generate_admin_cancel_keyboard(bookings: List[Dict[str, Any]]) -> InlineKeyboardMarkup:
    ...
    markup = InlineKeyboardMarkup(row_width=1) # Каждая бронь в своем ряду
    if not bookings:
        no_bookings_button = InlineKeyboardButton("Нет активных или будущих бронирований", callback_data=const.CB_IGNORE)
        markup.add(no_bookings_button)
        return markup
    ...
    for booking in bookings:
        ...
        if not all([b_id, b_date, b_start, b_end]):
            logger.warning(f"Неполные данные для бронирования {b_id} при генерации админ-клавиатуры отмены: {booking}")
            continue
        ...
        cr_display = cr_name[:max_name_len] + '..' if len(cr_name) > max_name_len else cr_name # Используем имя комнаты
        btn_text = f"ID:{b_id} {user_display} | {cr_display} | {date_str} {start_str}-{end_str}"
...
def generate_filter_options_keyboard() -> InlineKeyboardMarkup:
    ...
    # Обновляем кнопку для комнат
    cr_button = InlineKeyboardButton("🚪 По переговорной", callback_data=f"{const.CB_FILTER_BY_TYPE}cr") # Используем 'cr' как тип фильтра
    dates_button = InlineKeyboardButton("🗓️ По дате (месяц)", callback_data=f"{const.CB_FILTER_BY_TYPE}dates")
    markup.add(users_button)
    markup.add(cr_button) # Добавляем кнопку для комнат
...
def generate_filter_selection_keyboard(options: List[Tuple[Any, Any]], callback_prefix: str) -> InlineKeyboardMarkup:
    """Клавиатура выбора значения для фильтра /all (пользователь, комната, дата)."""
    ...
    for text, val in options:
        ...
        # Префикс будет, например, const.CB_FILTER_SELECT_USER, const.CB_FILTER_SELECT_CR, const.CB_FILTER_SELECT_DATE
        button = InlineKeyboardButton(display_text, callback_data=f"{callback_prefix}{val}")
        markup.add(button)

    # Кнопка "Назад" к выбору типа фильтра
    cancel_button = InlineKeyboardButton("🔙 Назад к выбору фильтра", callback_data=f"{const.CB_ACTION_CANCEL}filter_type")
    markup.add(cancel_button)
...
def generate_extend_time_keyboard(booking_id: int, max_duration: Optional[timedelta] = None) -> InlineKeyboardMarkup:
    ...
    if limit_duration is None or limit_duration < timedelta(0):
        limit_duration = timedelta(0) # Если макс.длительность не передана или отрицательна, продление невозможно

    time_step = timedelta(minutes=const.BOOKING_TIME_STEP_MINUTES)

    # Генерируем кнопки, пока текущее время продления меньше или равно лимиту и больше нуля
    ...
    if not buttons:
        ...
        # Используем часть префикса как контекст для кнопки отмены
        cancel_context = const.CB_EXTEND_SELECT_BOOKING.replace('cb_', '', 1).replace('_s_book_', '', 1)
...
# --- END OF FILE utils/keyboards.py ---
...
## 2025-12-04 — Исправление автоматического обновления тепловой карты
- **Проблема:** Тепловая карта на вкладке "Бронирование оборудования" не обновлялась автоматически при переходе на вкладку после отмены бронирования. Требовалось нажимать F5 или кнопку "Показать занятость" вручную. Попытки автоматического обновления через сложную систему множественных попыток и прямых fetch-запросов не работали надежно.
- **Подпроект:** WSB_portal
- **Влияние:** Все пользователи, удобство использования интерфейса, актуальность данных на тепловой карте
- **Причина:** Сложная логика обновления с множественными попытками и прямыми fetch-запросами не учитывала все нюансы инициализации элементов и могла вызываться до готовности DOM.
- **Решение:** 
  - Упрощена логика обновления: при переходе на вкладку "Бронирование оборудования" программно нажимается кнопка "Показать занятость" через 300ms после переключения
  - Удалена сложная система множественных попыток и прямых fetch-запросов
  - Сохранена обработка отложенного обновления (после отмены бронирования) и установка даты по умолчанию
  - Используется вся существующая логика обновления из обработчика кнопки, что гарантирует корректную работу
- **Статус:** выполнено.

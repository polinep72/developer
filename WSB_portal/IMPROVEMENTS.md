...
**Реализация:** Добавлена вкладка "Календарь бронирований" с месячным календарем, API endpoint `/api/bookings/calendar`, функция `get_calendar_overview()` для получения статистики. Календарь кликабелен - при клике на дату открывается "Управление бронированием" с выбранной датой.
...
### 7. Автоматическая остановка предыдущих экземпляров сервера
**Приоритет:** Средний  
**Статус:** ✅ Реализовано  
**Описание:** При запуске сервера автоматически останавливать все предыдущие экземпляры на порту 8090 для предотвращения конфликтов.  
**Реализация:** Добавлена функция `stop_previous_instances()` в `app/main.py`, которая вызывается при старте приложения. Функция работает на Windows (netstat + taskkill) и Linux/Mac (lsof + kill).

### 8. Расширенное управление пользователями
**Приоритет:** Низкий  
...
5. Безопасность
6. ✅ Автоматическая остановка предыдущих экземпляров сервера

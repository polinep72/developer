...
- Приложение поддерживает двунаправленную конвертацию:
  - **PDF → MD**: Использует библиотеку `marker-pdf` для конвертации PDF в Markdown
  - **MD → PDF**: Использует `markdown` и `weasyprint` для конвертации Markdown в PDF
- Поддерживает обработку как отдельных файлов, так и архивов ZIP

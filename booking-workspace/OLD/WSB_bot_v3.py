...
"1": "Анализатор спектра R3465",
"2": "Анализатор спектра СК4М-18",
"3": "Анализатор спектра Aaronia 250XA-6",
"4": "Анализатор спектра портативные Signal Hound USB-SA44B",
"5": "Анализатор фазовых шумов PNA20",
"6": "Анализатор цепей векторные Р4М-18",
"7": "Анализатор цепей векторные ZNA43",
"8": "Генератор сигналов HMC-T2000",
"9": "Генератор сигналов векторные Aaronia V6-VSG120X",
"10": "Генератор сигналов скалярные PLG20-12F",
"11": "Генератор сигналов скалярные Advantex SGU21(до 21ГГц)",
"12": "Генератор сигналов скалярные N5183B(до 40ГГц)",
"13": "Генератор сигналов скалярные Advantex SG8(до 8ГГц)",
"14": "Генератор сигналов специальной формы SFG-2110(№1)",
"15": "Генератор сигналов специальной формы SFG-2110(№2)",
"16": "Зондовая станция ЭМ6030",
"17": " Осциллограф модульный U2702A",
"18": " Осциллограф цифровой DSO-X 3032A",
"19": " Осциллограф цифровые DSO-X 3034A",
"20": " Осциллограф цифровые MSOX3102A",
"21": "Осциллограф DSO4254C",
"22": "Осциллограф цифровой MSO5104",
"23": "Измеритель мощности СВЧ М3М-18",
"24": "Частотомер универсальные GFC-8270H",
"25": "Стенд большой мощности"
}
...
def handle_message(message):
    bot.reply_to(message, "Команды бота:\n"
        ...
        "/cancel - предлагает варианты для удаления резервирования оборудования.\n"
        "/finish - окончание работы на зарезервированном оборудовании.\n"
        "По окончании работы на рабочем месте необходимо нажать на эту кнопку для того чтобы освободить время резервирования. \n"
        "/users - просмотр зарегистрированных пользователей.\n"
        "/mybookings  - просмотр ваших резервирований оборудования.\n"
        "/workspacebookings - просмотр резервирований конкретного оборудования.\n"
...
def save_registered_user(user_id, first_name, last_name):
    with open(REGISTRATION_FILE, mode='a', newline='', encoding='utf-8') as file:
        ...
        writer.writerow([user_id, first_name, last_name, datetime.now().strftime("%d-%m-%Y %H:%M")])
...
def start_booking(message):
    ...
    bot.send_message(message.chat.id, "Выберите оборудование:", reply_markup=markup)
...
def select_workspace(call):
    ...
    bot.send_message(call.message.chat.id, f"Вы выбрали оборудование: {available_workspaces[workspace_id]}")
...
def select_date(call):
    ...
    markup = InlineKeyboardMarkup()
    time_one = [(8, 0), (8, 30), (9, 0), (9, 30), (10, 0), (10, 30), (11, 0), (11, 30), (12, 0), (12, 30), (13, 0), (13, 30), (14, 0), (14, 30), (15, 0), (15, 30), (16, 0), (16, 30), (17, 0), (17, 30), (18, 0), (18, 30), (19, 0), (19, 30)]
    for hours, minutes in time_one:  # Время с 08:00 до 20:00
...
def select_duration(call):
    ...
    first_name = registered_users[user_id]['first_name']
    ...
    if any(b_date == date and ws_id == workspace_id and 
        ...
        bot.send_message(call.message.chat.id, f"Ваше бронирование оборудования '{available_workspaces[workspace_id]}' подтверждено.")
...
def handle_cancel_booking(call):
    ...
    if booking_to_cancel:
        ...
        bot.send_message(call.message.chat.id, f"Ваше бронирование оборудования {available_workspaces[workspace_id]} на {date} с {time_interval} было успешно отменено.")
...
def handle_finish_booking(call):
    ...
    if booking_to_finish:
        ...
        bot.send_message(call.message.chat.id, f"Ваше бронирование оборудования {available_workspaces[workspace_id]} на {date} было завершено. Время окончания: {current_time}.")
...
def extend_booking(message):
    ...
    if user_booking:
        ...
        bot.send_message(message.chat.id, f"Ваше бронирование оборудования {available_workspaces[workspace_id]} активно до {booking_end_time}. Выберите время продления.")
...
def handle_extend_time(call):
    ...
    with open('bookings.csv', 'a', newline='', encoding='utf-8') as file:
        ...
        first_name = registered_users[user_id]['first_name']
        last_name = registered_users[user_id]['last_name']
        writer.writerow([user_id, first_name, last_name, workspace_id, datetime.now().strftime('%d-%m-%Y'), new_time_interval])
...
def view_workspace_bookings(call):
    ...
    if workspace_bookings:
        response = f"Бронирование оборудования {available_workspaces[workspace_id]} на ближайшую неделю:\n"
        # Формируем список бронирований
        ...
        bot.send_message(call.message.chat.id, f"На оборудовании {available_workspaces[workspace_id]} нет активных бронирований на ближайшую неделю.")
...
def view_bookings_by_date(call):
    ...
    if bookings_by_date:
        ...
        for booking in bookings_by_date:
            response += f"{booking['first_name']} {booking['last_name']} - Оборудование {available_workspaces[booking['workspace_id']]} с {booking['time_interval']}\n"
        bot.send_message(call.message.chat.id, response)
    else:
...
def send_welcome(message):
    ...
    # Приветствие
    bot.send_message(message.chat.id, "Привет! Я бот для резервирования оборудования. Используй /help для получения списка команд.", reply_markup=markup)

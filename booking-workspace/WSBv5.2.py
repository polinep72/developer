...
# Словарь для отслеживания подтверждённых бронирований
...
def handle_message(message):
    bot.reply_to(message, "Команды для администратора:\n"
                          ...
                          "/all - просмотр всех резервирований оборудования.\n"
...
def notify_user_about_booking(booking_id, user_id, chat_id, equip_name, start_time):
    ...
    # Запускаем таймер для автоматической отмены бронирования
    timer = threading.Timer(300, auto_cancel_booking, args=[booking_id, chat_id])  # 5 минут
    timer.start()

def auto_cancel_booking(booking_id, chat_id):
    # Проверяем, было ли бронирование подтверждено
    if booking_id in confirmed_bookings:
        # Бронирование уже подтверждено, отмена не требуется
        return
    ...
    """
    response3 = execute_query(query3, (booking_id,))

    for booking in response3:
        id, user_name, equip_name, date, time_start, time_end = booking

    update_query = "UPDATE bookings SET cancel = TRUE WHERE id = %s"
    execute_query1(update_query, (booking_id,))
    schedule_notifications()
    bot.send_message(chat_id, f"Ваше бронирование на оборудовании {equip_name} с началом работы {time_start.strftime('%H:%M')} было автоматически отменено из-за отсутствия подтверждения.")

@bot.callback_query_handler(func=lambda call: call.data.startswith("book_confirm_"))
def confirm_booking(call):
    booking_id = int(call.data.split("_")[2])

    # Добавляем бронирование в подтверждённые
    confirmed_bookings.add(booking_id)
...
def schedule_notifications():
    ...
    query = """SELECT b.id, b.user_id, b.equip_id, b.date, b.time_start, b.time_end, b.extension, e.name_equip AS equipment_name \
               FROM bookings b JOIN equipment e ON b.equip_id = e.id \
               WHERE b.cancel = false AND b.finish IS NULL AND b.date >= now()::date - interval '1h'"""
    ...
    bookings_df = bookings_df.rename(
        columns={0: 'id', 1: 'user_id', 2: 'workspace_id', 3: 'date', 4: 'time_start', 5: 'time_end', 6: 'extension', 7: 'name'})
    ...
    for index, booking in bookings_df.iterrows():
        ...
        if booking_key not in sent_notifications['start']:
            ...
            if notification_time_start > now:
                scheduler.add_job(notify_user_about_booking, DateTrigger(run_date=notification_time_start),
                                  args=[booking['id'], booking['user_id'], booking['user_id'], booking['name'], start_time])
            ...
            if notification_time_end > now:
                scheduler.add_job(send_notification, DateTrigger(run_date=notification_time_end),
                                  args=[booking['user_id'],
                                        f"Напоминание: Ваша работа на оборудовании {booking['name']} завершится через 10 минут ({booking['time_end']})"])
...
def admin_cancel_booking(message):
    query = "SELECT is_admin FROM users WHERE users_id = %s"
    result = execute_query(query, (message.from_user.id,))
    if not result or not result[0][0]:
...
def handle_confirm_cancel_booking(call):
    ...
    query1 = """
        ...
        ORDER BY b.date, b.time_start;
    """
    response1 = execute_query(query1, (booking_id,))

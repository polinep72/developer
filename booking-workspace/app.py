...
########################################################################################
# Команда для просмотра бронирований по оборудованию
...
def get_workspace_bookings(call):
    ...
    try:
        ...
        """
        user_bookings = execute_query(query, (equipment_id, now))

        # Получаем имя оборудования
        equip_query = "SELECT name_equip FROM equipment WHERE id = %s"
        workspace_name = execute_query(equip_query, (equipment_id,))
...
def confirm_booking(call):
    ...
    execute_query1(query, (booking_id,))
...
def schedule_notifications():
    ...
    query = """SELECT b.id, b.user_id, b.equip_id, b.date, b.time_start, b.time_end, b.extension, e.name_equip AS equipment_name, b.cancel \
    ...
    if not bookings_df:  # Проверяем, есть ли бронирования
        print("Нет активных бронирований для уведомлений.")
        return

    bookings_df = pd.DataFrame(list(bookings_df))
    bookings_df = bookings_df.rename(
        columns={0: 'id', 1: 'user_id', 2: 'workspace_id', 3: 'date', 4: 'time_start', 5: 'time_end', 6: 'extension',
                 7: 'name', 8: 'cancel'})
    ...
    for index, booking in bookings_df.iterrows():
        ...
        extension = pd.to_datetime(booking['extension'], format='%H:%M:%S') if booking['extension'] else None
        extension = timedelta(hours=extension.hour, minutes=extension.minute,
                              seconds=extension.second) if extension else timedelta(0)

        end_time += extension

        booking_key = (
        booking['user_id'], booking['workspace_id'], booking['date'], booking['time_start'], booking['time_end'])
        ...
        if booking_key not in sent_notifications['start']:
            ...
            if notification_time_start > now:
                scheduler.add_job(notify_user_about_booking, DateTrigger(run_date=notification_time_start),
                                  args=[booking['id'], booking['user_id'], booking['user_id'], booking['name'],
                                        start_time])

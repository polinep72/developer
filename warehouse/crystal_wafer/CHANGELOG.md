...
- Исправлено применение фильтров в SQL запросах инвентаризации (добавлено WHERE 1=1 перед условиями фильтрации)
- Исправлено отображение сообщения "Нулевые остатки" - теперь используется тот же размер шрифта (20px), что и заголовок страницы

### Changed
- Улучшена логика определения статуса инвентаризации:
  - `no_results` - записи не найдены (отображается "По вашему запросу ничего не найдено")
  - `zero_stock` - записи найдены, но все остатки = 0 (отображается "Нулевые остатки" с доступными кнопками экспорта)
  - `has_stock` - есть ненулевые остатки (отображается таблица с данными)
- Кнопки "Выгрузить в Excel" и "Сырые данные" теперь доступны даже при нулевых остатках (если записи найдены)
- Экспорт инвентаризации теперь включает все записи (включая с нулевыми остатками), а в таблице отображаются только записи с ненулевыми остатками
- Убран фильтр по остаткам из SQL запроса инвентаризации, чтобы возвращать все записи для корректной обработки нулевых остатков

...
# --- ПУЛ ПОДКЛЮЧЕНИЙ К БД СКЛАДА ГОТОВОЙ ПРОДУКЦИИ (warehouse) ---
_warehouse_db_pool = None
_warehouse_pool_lock = threading.Lock()


def init_warehouse_db_pool():
    """Инициализация пула подключений к БД warehouse (склад готовой продукции)"""
    global _warehouse_db_pool
    if _warehouse_db_pool is None:
        with _warehouse_pool_lock:
            if _warehouse_db_pool is None:
                db_name = os.getenv('WAREHOUSE_DB_NAME', 'warehouse')
                db_host = os.getenv('WAREHOUSE_DB_HOST', os.getenv('DB_HOST'))
                db_user = os.getenv('WAREHOUSE_DB_USER', os.getenv('DB_USER'))
                db_password = os.getenv('WAREHOUSE_DB_PASSWORD', os.getenv('DB_PASSWORD'))
                db_port = os.getenv('WAREHOUSE_DB_PORT', os.getenv('DB_PORT', '5432'))

                try:
                    _warehouse_db_pool = pool.ThreadedConnectionPool(
                        minconn=1,
                        maxconn=20,
                        host=db_host,
                        database=db_name,
                    ...
                    try:
                        _flask_app.logger.info(
                            f"Пул подключений к БД warehouse инициализирован: "
                            f"minconn=1, maxconn=20, host={db_host}, database={db_name}"
                        )
                    except (AttributeError, NameError):
                        print(
                            f"INFO: Пул подключений к БД warehouse инициализирован: "
                            f"minconn=1, maxconn=20, host={db_host}, database={db_name}"
                        )
                except Exception as e:
                    try:
                        _flask_app.logger.error(f"Ошибка инициализации пула warehouse: {e}", exc_info=True)
                    except (AttributeError, NameError):
                        print(f"ERROR: Ошибка инициализации пула warehouse: {e}")
                    raise
    return _warehouse_db_pool


def close_warehouse_db_pool():
    """Закрытие пула подключений к БД warehouse при завершении приложения"""
    global _warehouse_db_pool
    if _warehouse_db_pool:
        try:
            _warehouse_db_pool.closeall()
            try:
                _flask_app.logger.info("Пул подключений к БД warehouse закрыт")
            except (AttributeError, NameError):
                print("INFO: Пул подключений к БД warehouse закрыт")
        except Exception as e:
            try:
                _flask_app.logger.error(f"Ошибка при закрытии пула warehouse: {e}", exc_info=True)
            except (AttributeError, NameError):
                print(f"ERROR: Ошибка при закрытии пула warehouse: {e}")
        finally:
            _warehouse_db_pool = None
...
def get_warehouse_db_connection():
    """
    Получение подключения к БД warehouse из пула подключений
    """
    try:
        db_pool = init_warehouse_db_pool()
        conn = db_pool.getconn()
        return conn
    except pool.PoolError as e:
        _flask_app.logger.error(f"Ошибка получения подключения из пула warehouse: {e}", exc_info=True)
        raise
    except Exception as e:
        _flask_app.logger.error(f"Неожиданная ошибка при получении подключения к warehouse: {e}", exc_info=True)
        raise


def return_warehouse_db_connection(conn):
    """
    Возврат подключения к БД warehouse в пул
    """
    global _warehouse_db_pool
    if _warehouse_db_pool and conn:
        try:
            _warehouse_db_pool.putconn(conn)
        except Exception as e:
            _flask_app.logger.error(f"Ошибка возврата подключения в пул warehouse: {e}", exc_info=True)
            try:
                conn.close()
            except:
                pass
...
def execute_warehouse_query(query, params=None, fetch=True):
    """
    Выполнение SQL запроса к БД warehouse (склад готовой продукции)
    """
    conn = None
    start_time = time.time()
    try:
        conn = get_warehouse_db_connection()
        cur = conn.cursor()
        ...
        if execution_time > 1.0:
            _flask_app.logger.warning(
                f"Медленный запрос к warehouse ({execution_time:.2f}с): {query[:200]}... "
                f"Параметры: {params if params else 'None'}"
            )
        ...
        execution_time = time.time() - start_time
        _flask_app.logger.error(
            f"Ошибка при выполнении SQL в warehouse ({execution_time:.2f}с): {error}. "
            f"Запрос: {query[:500]}, Параметры: {params}",
            exc_info=True
        )
        if conn:
        ...
        if conn:
            return_warehouse_db_connection(conn)

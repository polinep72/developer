...
def get_bookings_by_date_text(db: Database, selected_date: date) -> str:
    ...
    if not bookings: return f"Нет бронирований на {date_str}."
...
def calculate_available_slots(
    ...
    # <<< ИЗМЕНЕНО: Определяем самое раннее время начала СЕГОДНЯ >>>
    today = datetime.now().date()
    is_today = (selected_date == today)
    now_dt = datetime.now()
    earliest_start_dt_today = now_dt # Начальное значение
    ...
    if is_today:
        ...
        # Учитываем секунды и микросекунды, чтобы избежать проблем при сравнении
    ...
    # Это либо начало рабочего дня, либо самое раннее время сегодня, если оно позже
    ...
    current_time_dt = effective_start_dt # Начинаем с эффективного старта
    ...
    for booking in sorted_bookings:
        ...
        # <<< ИЗМЕНЕНО: Проверяем слот перед бронью, начиная с current_time_dt >>>
        if booking_start_dt > current_time_dt:
            potential_slot_start_dt = current_time_dt # Начало возможно раньше, чем booking_start_dt
            potential_slot_end_dt = booking_start_dt  # Конец слота - начало брони

            # Убедимся, что слот не заканчивается раньше, чем начинается, и имеет минимальную длину
            if potential_slot_end_dt > potential_slot_start_dt and \
               (potential_slot_end_dt - potential_slot_start_dt) >= min_step_delta:

                # Ограничиваем рабочими часами (хотя current_time_dt уже должен быть в их пределах)
                ...
                # Дополнительная проверка после обрезки по рабочим часам
        ...
        # Передвигаем текущее время на конец текущей брони (или позже, если были пересечения)
        current_time_dt = max(current_time_dt, booking_end_dt)
        # <<< ИЗМЕНЕНО: Если сегодня, убедимся, что current_time_dt не раньше earliest_start_dt_today >>>
        if is_today:
            current_time_dt = max(current_time_dt, earliest_start_dt_today)


    # <<< ИЗМЕНЕНО: Проверяем промежуток после последней брони до конца рабочего дня >>>
    if work_end_dt > current_time_dt:
        potential_slot_start_dt = current_time_dt # Начинаем сразу после последней брони (или с earliest_start_dt_today)
        potential_slot_end_dt = work_end_dt     # До конца рабочего дня

        if potential_slot_end_dt > potential_slot_start_dt and \
           (potential_slot_end_dt - potential_slot_start_dt) >= min_step_delta:

            # Ограничиваем рабочими часами (на всякий случай)
            slot_start_time = max(potential_slot_start_dt.time(), work_start_time)
            slot_end_time = min(potential_slot_end_dt.time(), work_end_time)

            # Дополнительная проверка после обрезки
            if datetime.combine(selected_date, slot_end_time) > datetime.combine(selected_date, slot_start_time) and \
               (datetime.combine(selected_date, slot_end_time) - datetime.combine(selected_date, slot_start_time)) >= min_step_delta:
                available_slots.append((slot_start_time, slot_end_time))

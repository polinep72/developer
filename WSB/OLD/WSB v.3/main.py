...
import telebot # <<< Оставил один импорт telebot здесь

# --- Импорт основных компонентов ---
from config import TELEGRAM_BOT_TOKEN, SCHEDULER_TIMEZONE
from database import Database
from logger import logger
from apscheduler.schedulers.background import BackgroundScheduler
from typing import Dict, Any, Set, Tuple
from bot_app import bot, db_connection, scheduler, active_timers, scheduled_jobs_registry # <-- ДОБАВИТЬ scheduled_jobs_registry
...
# db_connection = Database() # Инициализируется в bot_app
# scheduler = BackgroundScheduler(timezone=SCHEDULER_TIMEZONE) # Инициализируется в bot_app
active_timers_main: Dict[int, Any] = {} # Используется ли эта переменная? active_timers импортируется из bot_app
# scheduled_jobs_registry_main: Set[Tuple[str, int]] = set() # Используется ли эта переменная? scheduled_jobs_registry импортируется из bot_app
# bot = telebot.TeleBot(TELEGRAM_BOT_TOKEN) # Инициализируется в bot_app
...
def main():
    ...
    try:
        ...
        callback_handlers.register_callback_handlers(
            bot, db_connection, scheduler, active_timers, scheduled_jobs_registry  # <-- Использовать импортированный
        )
        ...
        while not shutdown_flag:
            ...
            try:
                ...
                # Обработка таймаута чтения от Telegram API
                logger.warning("Таймаут чтения от Telegram API. Перезапуск polling через 30 секунд...")
                ...
                logger.error(f"Ошибка сетевого подключения: {ce}. Перезапуск polling через 90 секунд...")
                # Сбрасываем счетчик ошибок 409
                api_409_retries = 0
                # Сбрасываем задержку для 409
                current_409_delay = INITIAL_RETRY_DELAY
...
if __name__ == "__main__":
    ...
    # logging.getLogger('apscheduler.executors.default').setLevel(logging.WARNING) # <-- Закомментировать или изменить на INFO/DEBUG
    # logging.getLogger('apscheduler.scheduler').setLevel(logging.WARNING)       # <-- Закомментировать или изменить на INFO/DEBUG
    logging.getLogger('apscheduler').setLevel(logging.DEBUG)  # <-- Добавить эту строку для детального лога

...
def schedule_one_notification(
        ...
        job_type: str,
        booking_id: int,
        run_time: datetime, # Время из БД (предполагаем naive Moscow Time)
        func_to_run: Callable,
        args_for_func: list
):
    """
    Планирует одно уведомление в APScheduler, если оно еще не запланировано.
    Корректно обрабатывает naive datetime из БД как время в целевой таймзоне.
    Удаляет существующую задачу с тем же ID перед добавлением новой.
    """
    job_key = (job_type, booking_id)
    job_id = f"{job_type}_{booking_id}"
    run_time_aware = None # Время в целевой таймзоне (Europe/Moscow)

    try:
        target_timezone = scheduler.timezone # Europe/Moscow
        now_aware = datetime.now(target_timezone)

        # --- ИСПРАВЛЕННАЯ ЛОГИКА ОБРАБОТКИ ВРЕМЕНИ (v2) ---
        run_time_tz_info = run_time.tzinfo
        if run_time_tz_info is None:
            # Считаем наивное время из БД временем в ЦЕЛЕВОЙ таймзоне (Europe/Moscow)
            try:
                # Локализуем наивное время с помощью целевого часового пояса
                run_time_aware = target_timezone.localize(run_time)
                logger.debug(f"Время {run_time} из БД - наивное. Локализовано как {target_timezone.zone}: {run_time_aware.strftime('%Y-%m-%d %H:%M:%S %Z%z')}")
            except Exception as e_tz_localize:
                 logger.error(f"Ошибка локализации наивного времени {run_time} в {target_timezone.zone} для задачи {job_id}: {e_tz_localize}", exc_info=True)
                 # Не можем продолжить без корректного времени
                 return
        else:
            # Если время из БД уже aware (маловероятно), приводим к поясу планировщика
            logger.warning(f"Время {run_time} из БД неожиданно имеет tzinfo ({run_time_tz_info}). Приводим к {target_timezone.zone}.")
        ...
        if run_time_aware <= now_aware:
            logger.debug(f"Время запуска задачи {job_id} ({run_time_aware.strftime('%Y-%m-%d %H:%M:%S %Z%z')}) уже прошло ({now_aware.strftime('%Y-%m-%d %H:%M:%S %Z%z')}). Пропуск планирования.")
        ...
        if job_in_registry:
            if existing_job:
                ...
                if existing_run_time == run_time_aware:
                    ...
                    existing_run_time_str = existing_run_time.strftime('%Y-%m-%d %H:%M:%S %Z%z') if existing_run_time else 'N/A'
                    required_run_time_str = run_time_aware.strftime('%Y-%m-%d %H:%M:%S %Z%z')
                    logger.warning(f"Задача {job_id} в реестре, но время в планировщике ({existing_run_time_str}) отличается от требуемого ({required_run_time_str}). Перепланируем.")
                    try:
        ...
        # Удаляем старую задачу из планировщика, если она там была
        if existing_job:
            try:
        ...
        # Логирование перед добавлением задачи
        logger.info(
            f"ПЛАНИРОВАНИЕ: Job ID={job_id}, "
            f"Run Time (Aware)={run_time_aware.strftime('%Y-%m-%d %H:%M:%S %Z%z')}, "
            f"Current Time (Aware)={now_aware.strftime('%Y-%m-%d %H:%M:%S %Z%z')}"
        )

        # Добавляем новую задачу с корректным aware временем
        scheduler.add_job(
            func_to_run,
        ...
        try:
            ...
            # Используем aware время для лога
            logger.info(f"Запланирована задача {job_id} на {run_time_aware.strftime('%Y-%m-%d %H:%M:%S %Z%z')}")
...
def notify_user_about_booking_start(
    ...
    actual_run_time = datetime.now()  # Получаем реальное время запуска
    logger.info(f"ЗАПУСК notify_start для брони {booking_id}. "
f"Реальное время: {actual_run_time.strftime('%Y-%m-%d %H:%M:%S.%f')}. "
f"Запланировано на (UTC): {start_time.astimezone(pytz.utc).strftime('%Y-%m-%d %H:%M:%S %Z%z')}. " # Логируем в UTC для сравнения
f"Запланировано на (Local): {start_time.strftime('%Y-%m-%d %H:%M:%S')}" # Логируем исходное
                )
    # ... остальная логика ...
...
# --- START OF MODIFIED FUNCTION send_end_booking_notification_wrapper ---
# В services/notification_service.py
...
def send_end_booking_notification_wrapper(
        ...
        booking_id: int,
        user_id: int,
        room_id: int, # Адаптировано для CRB
        room_name: str, # Адаптировано для CRB
        end_time: datetime
):
    """Проверяет возможность продления и отправляет уведомление об окончании (за N минут)."""
    # --- ДОБАВЛЕНО ОПРЕДЕЛЕНИЕ ПЕРЕМЕННОЙ ---
    actual_run_time = datetime.now() # Получаем реальное время запуска
    # --- КОНЕЦ ДОБАВЛЕНИЯ ---

    # Используем переменную в логе
    logger.info(
        f"ЗАПУСК notify_end для брони {booking_id}. "
        f"Реальное время: {actual_run_time.strftime('%Y-%m-%d %H:%M:%S.%f')}. "
        f"Запланировано на (UTC): {end_time.astimezone(pytz.utc).strftime('%Y-%m-%d %H:%M:%S %Z%z')}. "
        f"Запланировано на (Local): {end_time.strftime('%Y-%m-%d %H:%M:%S')}"
    )
    # --- Дальнейший код функции остается без изменений ---
    booking_info: Optional[Dict[str, Any]] = None
    notification_message_id: Optional[int] = None
    end_time_aware = None
    ...
    try:
        ...
        # 2. Приведение времени к aware
        try:
            if end_time.tzinfo is None:
                end_time_aware = end_time.replace(tzinfo=scheduler.timezone)
            else:
                end_time_aware = end_time.astimezone(scheduler.timezone)
        except Exception as e_tz:
             logger.error(f"Ошибка приведения end_time ({end_time}) к часовому поясу для брони {booking_id}: {e_tz}")
             end_time_aware = end_time

        # 3. Проверка возможности продления
        can_extend = False
        try:
            check_start_time = end_time_aware # Используем aware время для проверки
            ...
            conflicts = booking_service.check_booking_conflict(
                db, room_id, # Используем room_id
                check_start_time.replace(tzinfo=None), # Передаем naive для запроса в БД
                check_end_time.replace(tzinfo=None),   # Передаем naive для запроса в БД
                exclude_booking_id=booking_id
            )
            if not conflicts:
                # Сравниваем aware время с aware концом рабочего дня
                ...
                if check_start_time < end_work_dt:
                    ...
                    logger.debug(f"Продление {booking_id} невозможно (конец раб. дня).")
            else:
                logger.debug(f"Продление {booking_id} невозможно (конфликт).")
        ...
        if can_extend:
            message_text = (
                f"🔔 Напоминание: Ваше использование переговорной '{room_name}' завершится через {minutes_left} мин ({end_time_str}).\nХотите продлить?")
        ...
        # 5. Запуск таймера для изменения сообщения
        if notification_message_id:
            if can_extend:
                ...
                if booking_id not in active_timers:
                    ...
                    timer = threading.Timer(
                        ...
                        args=[bot, user_id, notification_message_id, room_name, booking_id, end_time_aware, scheduler, scheduled_jobs_registry, active_timers]
                    )
                    active_timers[booking_id] = timer
                    timer.start()
            ...
            # 6. Обновление аргументов для финального уведомления
            final_end_job_type = getattr(const, 'JOB_TYPE_FINAL_END_NOTICE', None)
            if final_end_job_type:
                ...
                try:
                    ...
                    if job:
                        ...
                        job.modify(args=[bot, user_id, room_name, booking_id, msg_id_to_pass])
                        logger.debug(f"Обновлены аргументы (message_id={msg_id_to_pass}) для задачи {job_id}.")
                    else:
                        logger.warning(f"Не найдена задача {job_id} для обновления аргументов.")
                except JobLookupError:
                    logger.warning(f"Задача {job_id} не найдена в планировщике при попытке обновить аргументы.")
                except Exception as e_modify_job:
                    logger.error(f"Ошибка при обновлении аргументов задачи {job_id}: {e_modify_job}", exc_info=True)
        else:

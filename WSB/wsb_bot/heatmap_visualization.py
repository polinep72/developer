...
def fetch_data_wsb(selected_date_str: str) -> Tuple[Optional[List[str]], Optional[np.ndarray], Optional[str]]:
    ...
    try:
        ...
        query_bookings = """
            SELECT b.equip_id, b.time_start, b.time_end, b.finish
            FROM bookings b
        ...
        now_naive_local = datetime.now() # Текущее реальное "наивное" время
        # Если selected_date_obj - это не сегодня, то now_naive_local для сравнения с db_time_start/end
        # должно быть приведено к этой дате, но с сохранением текущего времени, либо нужно
        # сравнивать db_time_start/end с now_naive_local только если selected_date_obj == date.today()
        # Для простоты, если смотрим прошлый день, считаем, что "сейчас" это конец того дня.
        # Если смотрим будущий день, считаем, что "сейчас" это начало того дня.
        # Это упростит определение "active_usage" vs "booked_future" для исторических/будущих дат.
        ...
        # Если selected_date_obj == date.today(), то effective_now_for_status_check = now_naive_local
        ...
        for booking in bookings:
            ...
            for i in range(num_time_slots):
                ...
                # Определяем, перекрывается ли слот с бронированием
                # Учитываем фактическое время завершения, если оно есть
                ...
                if is_overlapping:
                    determined_status_code = STATUS_CODES["free"] # Значение по умолчанию перед определением
                    ...
                    if db_finish_time:
                        ...
                        # else: остается free, если слот после фактического завершения
                    
                    # 2. Если бронь НЕ завершена фактически (db_finish_time is None)
                    else:
                        # Слот находится внутри планового времени [db_time_start, db_time_end)
                        if current_slot_start_dt >= db_time_start and current_slot_start_dt < db_time_end:
                            # Если плановое время окончания уже прошло (относительно effective_now)
                            if effective_now_for_status_check >= db_time_end:
                                determined_status_code = STATUS_CODES["finished_usage"] # Завершено по расписанию
                            # Если плановое время окончания еще не наступило
                            else:
                                # Если реальное время начала брони уже наступило (относительно effective_now)
                                if effective_now_for_status_check >= db_time_start:
                                    determined_status_code = STATUS_CODES["active_usage"]
                                else: # Реальное время начала брони еще НЕ наступило
                                    determined_status_code = STATUS_CODES["booked_future"]
                        # Если слот не строго внутри, но пересекается (например, начинается до db_time_start, но заканчивается после)
                        # и при этом не "active_usage" и не "finished_usage", то это "booked_future"
                        elif determined_status_code == STATUS_CODES["free"]: # Если статус еще не был определен как finished/active
                             if current_slot_start_dt < db_time_end and current_slot_end_dt > db_time_start: # есть пересечение
                                 if effective_now_for_status_check < db_time_start: # и бронь в будущем
                                     determined_status_code = STATUS_CODES["booked_future"]
                                 # elif effective_now_for_status_check >= db_time_end: # бронь в прошлом (уже должна быть finished)
                                     # determined_status_code = STATUS_CODES["finished_usage"]

                    # Обновляем, если новый статус "важнее" или ячейка свободна
                    if determined_status_code != STATUS_CODES["free"]: # Если статус не "free"
                        if usage_data[equip_idx][i] == STATUS_CODES["free"] or \
                           determined_status_code > usage_data[equip_idx][i]:
                            usage_data[equip_idx][i] = determined_status_code
                # else: слот не пересекается с этой бронью, остается usage_data[equip_idx][i] (изначально free)
        
        logger.info(f"Матрица занятости WSB для {selected_date_str} успешно сформирована.")
        return equipment_names_for_y_axis, usage_data, None

    except psycopg2.OperationalError as e_op:
        logger.error(f"Ошибка подключения к БД WSB: {e_op}", exc_info=True)
        return None, None, f"Ошибка подключения к БД ({e_op}). Проверьте креды и доступность БД."
    except Exception as e_fetch:
        logger.error(f"Неожиданная ошибка при получении данных WSB: {e_fetch}", exc_info=True)
        return None, None, f"Неожиданная ошибка: {e_fetch}"
    finally:
        if conn:
            conn.close()
...
def update_heatmap_wsb(selected_date_from_picker_str: Optional[str], url_search: Optional[str]):
    ...
    if triggered_input_id == 'url' and url_search:
        try:
            ...
            if date_from_url_list:
                ...
                logger.info(f"Дата '{final_selected_date_str}' успешно получена из URL.")
            else: 
                ...
                logger.debug(f"Параметр 'date' отсутствует в URL, используется дата: {final_selected_date_str}")
        except (ValueError, TypeError) as e_url:
            logger.warning(f"Не удалось обработать дату из URL search '{url_search}': {e_url}. Используется дата из DatePicker или дефолтная.")
            final_selected_date_str = selected_date_from_picker_str if selected_date_from_picker_str else default_date_obj.strftime('%Y-%m-%d')
    else: 
        final_selected_date_str = selected_date_from_picker_str if selected_date_from_picker_str else default_date_obj.strftime('%Y-%m-%d')
    ...
    try:
        ...
        logger.error(f"Финальная строка даты '{final_selected_date_str}' некорректна, используется дефолтная.")
    ...
    equipment_names, usage_data, error_msg = fetch_data_wsb(final_selected_date_str)

    fig = go.Figure()
    fig.update_layout(
        title_text=f'Занятость оборудования на {final_selected_date_str}',
        xaxis={"visible": False, "title_text": "Время"}, 
        yaxis={"visible": False, "title_text": "Оборудование"}, 
        plot_bgcolor='white', paper_bgcolor='white'
    )

    if error_msg:
        logger.error(f"Ошибка при получении данных для {final_selected_date_str}: {error_msg}")
    ...
    if equipment_names is None or usage_data is None :
        no_data_msg = "Нет данных для отображения (внутренняя ошибка)."
        logger.error(f"Данные None для {final_selected_date_str} без явного error_msg.")
    ...
    if not equipment_names: 
        ...
        logger.info(f"Список оборудования пуст для {final_selected_date_str} (возможно, в БД нет оборудования).")
    ...
    logger.debug(f"Итоговая colorscale WSB: {final_colorscale}")
    logger.debug(f"Tick values для colorbar WSB: {tick_values_for_bar}")
    logger.debug(f"Tick texts для colorbar WSB: {tick_texts_for_bar}")
    ...
    fig_data_heatmap = go.Heatmap(
        ...
        colorbar=dict(
            ...
            tickmode='array',
            lenmode="pixels", len=max(150, num_defined_statuses * 35 + 40),

...
import logging
from database import execute_query, execute_query1
from notifications import schedule_notifications, send_notification
...
def auto_cancel_booking(bot, booking_id, chat_id, DB_CONFIG, schedule_notifications):
...
def finalize_booking(bot, call, DB_CONFIG, schedule_notifications, scheduler):
    ...
    if notification_time > datetime.now():
        ...
        logging.info(f"Уведомление о подтверждении бронирования {booking_id} запланировано на {notification_time}")

        # Планируем автоматическую отмену через 5 минут после уведомления, если нет подтверждения
        cancel_time = notification_time + timedelta(minutes=5)
        if cancel_time > datetime.now():
            scheduler.add_job(
                auto_cancel_booking,
                'date',
                run_date=cancel_time,
                args=[bot, booking_id, chat_id, DB_CONFIG, schedule_notifications]
            )

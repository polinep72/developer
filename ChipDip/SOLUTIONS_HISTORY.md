...
## 2025-10-29 - Публичные графики и мониторинг
**Проблема**: Графики 3 и 6 отсутствовали у неавторизованных пользователей; отсутствовал healthcheck
**Подпроект**: ChipDip
**Влияние**: Доступность статистики без логина, улучшение наблюдаемости
**Причина**: API были защищены `login_required`, отсутствовали маршруты `/health`
**Решение**: Снята авторизация с API выручки и экспортов; добавлены `GET /health`, `GET /api/health`, `GET /healthz`; пересоздан контейнер `webapp`
**Статус**: Завершено

### Детали решения:
- Публичные API: `/api/stats/revenue/monthly`, `/api/stats/revenue/product-monthly?product_id=...`
- Экспорт без авторизации для всех графиков
- Health‑эндпоинты для мониторинга приложения и БД
- Инструкции по пересозданию контейнера и логам в PowerShell
